...
// Assuming you have elements with the class 'default-btn'
var buttons = document.querySelectorAll('.default-btn');
...
buttons.forEach(function(button) {
  button.addEventListener('click', function(event) {
    ...
    if (!shoppingCart.itemExists(id)) {
      ...
      // Display an error message or handle the case where the item already exists
      alert('Item with ID ' + id + ' already exists in the cart!');
    }
  });
});
// Function to check if an item with a given id already exists in the cart
shoppingCart.itemExists = function(id) {
...
function displayCart() {
  ...
  // Check if the element with the class 'show-cart' exists before setting its innerHTML
  

  // Check if the elements with the classes 'total-cart' and 'total-count' exist before setting their innerHTML
  var totalCartElement = document.querySelector('.total-cart');
  var totalCountElement = document.querySelector('.total-count');

  if (totalCartElement) {
    totalCartElement.innerHTML = shoppingCart.totalCart();
  }

  if (totalCountElement) {
    totalCountElement.innerHTML = shoppingCart.totalCount();
  }

  // Assuming you have an element with the class 'show-cart'
  var showCartElement = document.querySelector('.show-cart');
  if (showCartElement) {
    showCartElement.innerHTML = output;
  }
}
